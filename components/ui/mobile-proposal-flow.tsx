...
export function MobileProposalFlow({ children, onComplete }: MobileProposalFlowProps) {
    ...
    const goToTab = (index: number) => {
        // Faqat tugallangan tablar yoki joriy tabdan keyingi tabga o'tishga ruxsat berish
        if (tabs[index].isCompleted || index === activeTabIndex || index === activeTabIndex + 1) {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex justify-between items-center px-4 py-3 bg-white shadow-sm">
                {tabs.map((tab, index) => (
                    <button
                        ...
                        className={`flex flex-col items-center ${index === activeTabIndex ? "text-primary-600" :
                            tab.isCompleted ? "text-green-600" : "text-gray-400"} 
              ${index > activeTabIndex && !tab.isCompleted ? "opacity-50" : ""}`}
                        disabled={index > activeTabIndex && !tabs[index - 1].isCompleted}
                    >
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center mb-1 
              ${index === activeTabIndex ? "bg-primary-100 text-primary-600" :
                                tab.isCompleted ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}>
                            {tab.isCompleted ? (
                ...
                <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                    <div className="bg-white p-6 rounded-xl shadow-lg max-w-sm text-center">
                        ...
                        <p className="text-gray-600 mb-4">
                            Taklifnomani to'liq ko'rish va yuklab olish uchun to'lovni amalga oshiring.
                        </p>
            ...
            {activeTabIndex < 2 && (
              ...
              ${!tabs[activeTabIndex].content ? "opacity-50" : "hover:bg-primary-700"} transition-colors`}
                    >
